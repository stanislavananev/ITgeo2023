...
def get_currency_data(request, currency_code):
    ...
    token = "Fjj0nmFI2vv2fH6xTULaE2tmbYqukgPPSK_Ly5HEyMZaiDPw2_c-YvOjFybWruw_qisJM4uUigKX1HuVAXl84w=="
    org = "nsu"
    ...
    flux_query = f'''
        from(bucket: "{bucket}")
          |> range(start: -30d)
          |> filter(fn: (r) => r._measurement == "exchange_rates" and r.currency == "{currency_code}")
        '''
    ...
    for table in result:
        for record in table.records:
            ...
            time = time.astimezone(pytz.timezone('Asia/Bangkok'))
            formatted_time = time.strftime('%Y-%m-%d %H:%M:%S') + '+00:00'
